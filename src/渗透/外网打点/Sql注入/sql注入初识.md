...
关联:
  - "[[知识/知识笔记/渗透/外网打点/Sql注入/Sql注入|Sql注入]]"
title: sql注入初识
---
# mysql注入
...
注意id的内容是直接进行拼接，没有进行任何处理，即对用户提交的数据完全信任。从注入的本质上来说就是没有区别出代码和数据
...
其实这样是不正确的的，因为你自己进行了尝试就会发现，输入1"#也可以登录，甚至)也可以。

所以为什么会发生这种原因，看其本质是在sql语句中，`''`包裹的语句会被识别为字符串，其中命令包括注释都不会执行，而查询为数字类型，就会取第一个字符数字，否则报错。
